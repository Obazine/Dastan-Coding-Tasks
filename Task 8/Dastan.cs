    ...
    class Dastan
    {
        ...
        public void PlayGame()
        {
            ...
            while (!GameOver)
            {
                ...
                if (CurrentPlayer.ChoiceIsSahm(CurrentPlayer.GetMoveOptionQueue().GetQueue()[Choice-1]) && !CurrentPlayer.GetSahmStatus())
                {
                    while (!SquareIsValid)
                    {
                        StartSquareReference = GetSquareReference("containing the piece to fire the Sahm");
                        SquareIsValid = CheckSquareIsValid(StartSquareReference, true);
                    }
                    int SahmScore = CalculateSahmMove(StartSquareReference, CurrentPlayer.GetDirection());
                    CurrentPlayer.ChangeScore(SahmScore);
                    CurrentPlayer.SetSahmUsed(true);
                    Console.WriteLine("New score: " + CurrentPlayer.GetScore() + Environment.NewLine);
                }
                else if(CurrentPlayer.ChoiceIsSahm(CurrentPlayer.GetMoveOptionQueue().GetQueue()[Choice - 1]))
                {
                    Console.WriteLine("You have already used a Sahm!");
                }
                else
                {
                    while (!SquareIsValid)
        ...
        private int CalculateSahmMove(int StartSquareReference, int Direction)
        {
            int row = StartSquareReference / 10;
            int column = StartSquareReference % 10;
            int ScoreGained = 0;
            if (Direction == 1)
            {
                for (int i = row + 1; i < 7; i++)
                {
                    int score = CalculatePieceCapturePoints((i * 10) + column);
                    ScoreGained += score;
                    if (!Board[GetIndexOfSquare((i * 10) + column)].ContainsKotla())
                    {
                        Board[GetIndexOfSquare((i * 10) + column)].RemovePiece();
                    }
                }
            }
            else
            {
                for (int i = row; i < 0; i--)
                {
                    int score = CalculatePieceCapturePoints(row * 10 + i);
                    ScoreGained += score;
                    if (!Board[GetIndexOfSquare((i * 10) + column)].ContainsKotla())
                    {
                        Board[GetIndexOfSquare((i * 10) + column)].RemovePiece();
