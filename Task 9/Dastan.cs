    ...
    class Dastan
    {
        ...
        public void PlayGame()
        {
            ...
            while (!GameOver)
            {
                ...
                do
                {
                    Console.Write("Choose move option to use from queue (1 to 3) or 7 to replace a piece with a Kotla or 9 to take the offer: ");
                ...
                if (Choice == 7)
                {
                    while (!SquareIsValid)
                    {
                        StartSquareReference = GetSquareReference("containing the piece to change to the Kotla");
                        SquareIsValid = CheckSquareIsValid(StartSquareReference, true);
                        if (!(Board[GetIndexOfSquare(StartSquareReference)].GetPieceInSquare().GetSymbol() == "K") || !(Board[GetIndexOfSquare(StartSquareReference)].GetPieceInSquare().GetSymbol() == "k"))
                        {
                            if (CurrentPlayer.SameAs(Players[0]))
                            {
                                Board[GetIndexOfSquare(StartSquareReference)].RemovePiece();
